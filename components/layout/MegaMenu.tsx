...
// ✅ SAFE Dynamic Icon Resolver (Production Ready)
const DynamicIcon = ({
  name,
  className,
}: {
...
export default function MegaMenu({ sections, onClose }: MegaMenuProps) {
  ...
  return (
    ...
    >
      {/* PANEL */}
      <div
        className={`rounded-lg overflow-hidden border shadow-2xl transition-colors duration-200 ${isDark
            ? "bg-[#0A0A12] border-[rgba(255,255,255,0.08)] shadow-black/60"
            : "bg-[#F7F5EF] border-gray-300 shadow-gray-300/40"
          }`}
      >
        ...
        >
          <span
            className={`text-[11px] font-black tracking-[0.15em] uppercase ${isDark ? "text-white" : "text-gray-700"
              }`}
          >
            {sectionTitle}
          </span>
